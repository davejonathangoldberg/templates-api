module.exports = function Composer(app) {
  ...
  // STORAGE TEST
  this.storagetest = function(req, res, next){
    console.log('bucket: ' + req.params['bucket']);
    res.set('content-type', 'application/json');
    res.status(200).json({ "bucket" : req.params['bucket']});
  }
  
  // ADD CLIENT
  this.addClient = function(data, callback){
    var clientId = Math.random().toString(36).slice(2);
    data['model'] = models['clients'];
    data['id'] = clientId;
    data['clientKey'] = clientId;
    coredb.insertItem(data, callback);
  };
  
  // CHECK CLIENT
  this.checkClient = function(data, callback){
    console.log('data: ' + JSON.stringify(data));
    data['model'] = models['clients'];
    data['query'] = { 'clientKey' : data.clientKey };
    coredb.findOneRecord(data, callback);
  };
  
  // LOOKUP BUCKET
  this.lookupBucket = function(bucketInfo, callback){
    bucketInfo['model'] = models['buckets'];
    bucketInfo['query'] = { 'bucketId' : bucketInfo.bucketId };
    coredb.findOneRecord(bucketInfo, callback);
  };
  
  this.getTemplatesForBucket = function(templatesInfo, callback){
    
  }
  
  // INSERT BUCKET
  this.insertBucket = function(bucketInfo, callback){
    bucketInfo['model'] = models['buckets'];
    bucketInfo['createdDate'] = new Date();
    bucketInfo['modifiedDate'] = new Date();
    coredb.insertItem(bucketInfo, callback);
  };
  
  // UPDATE BUCKET
  this.updateBucket = function(bucketInfo, callback){
    bucketInfo['model'] = models['buckets'];
    bucketInfo['query'] = { 'bucketId' : bucketInfo.bucketId };
    bucketInfo['updateQuery'] = {
      'bucketId'          : bucketInfo.bucketId,
      'readPermissions'   : bucketInfo.readPermissions,
      'writePermissions'  : bucketInfo.writePermissions,
      'metadata'          : bucketInfo.metadata || [],
      'templates'         : bucketInfo.templates || [],
      'createdDate'       : bucketInfo.createdDate || new Date(),
      'modifiedDate'      : bucketInfo.modifiedDate || new Date()
    }
    bucketInfo['options'] = { 'upsert' : true };
    coredb.updateItem(bucketInfo, function(err, updatedInfo){
      if (err) {
        console.error(err);
        var st_err = {
        ...
        if(updatedInfo.numberUpdated > 0){
          callback(null, bucketInfo['updateQuery']);
        } else {
          var st_err = {
            "name" : "Database Update Error",
            "code" : 500,
  ...
  this.insertTemplate = function(templateInfo, bucketInfo, callback){
    templateInfo['model'] = models['buckets'];
    templateInfo['options'] = { 'upsert' : true };
    templateInfo['query'] = { 'bucketId' : bucketInfo.bucketId };
    templateInfo['createdDate'] = new Date();
    templateInfo['modifiedDate'] = new Date();
    templateInfo['updateQuery'] = {
      "$push": {
        "templates" : {
          "templateId" : templateInfo.templateId,
          "templateBody" : templateInfo.templateBody,
          "templateMediaType" : templateInfo.templateMediaType,
          "templateLanguage" : templateInfo.templateLanguage,
          "paperFormat" : templateInfo.paperFormat,
          "paperOrientation" : templateInfo.paperOrientation,
          "paperWidth" : templateInfo.paperWidth,
          "paperHeight" : templateInfo.paperHeight,
    ...
    coredb.updateItem(templateInfo, function(err, updatedInfo){
      if (err) {
        console.error(err);
        return callback(err, "");
      } else {
  ...
  this.updateTemplate = function(templateInfo, bucketInfo, callback){
    templateInfo['model'] = models['buckets'];
    templateInfo['options'] = { 'upsert' : true };
    templateInfo['query'] = { 'bucketId' : bucketInfo.bucketId, 'templates.templateId' : templateInfo.templateId };
    templateInfo['createdDate'] = new Date();
    templateInfo['modifiedDate'] = new Date();
    templateInfo['updateQuery'] = {
      "$set": {
